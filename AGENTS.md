...
- `backend/` — Express API runtime, HTTP surface, middleware, Exa client, and agent pipelines
  - `backend/src/db/` — SQLite connection, migrations, parser, execution safety
  - `backend/src/llm/` — Unified LLM client with retry, timeout, streaming
  - `backend/src/config/` — Config store, key masking, test routes
  - `backend/src/routes/` — Express routers, API endpoints, SSE streaming
  - `backend/src/agents/` — LangGraph graph definitions, pipeline orchestration
  - `backend/src/skills/` — AgentSkill implementations, prompts, Zod schemas
  - `backend/src/wiki/` — File-system wiki store, path safety
- `frontend/` — React 18 SPA, Vite build, Tailwind, routing
  - `frontend/src/components/` — UI components, panels, modals, graph visualization
  - `frontend/src/pages/` — Route-level page components
  - `frontend/src/context/` — React context providers
- `shared/` — Cross-cutting TypeScript types, Zod schemas, interfaces
- `plan/` — Sprint docs, requirements, planning artifacts, datasets
